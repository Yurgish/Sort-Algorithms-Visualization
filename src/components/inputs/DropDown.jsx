  ...
  return (
    <div className={`relative ${className}`}>
      ...
      <div className="absolute top-[2px] right-[2px] h-4">
        <button
          className={`w-[15px] h-full bg-window-background flex items-center justify-center ${
            isOpen ? "shadow-window-reverse" : "shadow-window"
          }`}
          onClick={() => setIsOpen(!isOpen)}
        >
          <img src={ArrowDownIcon} className="scale-150" />
        </button>
      </div>
      {isOpen && (
        <ul className="absolute left-0 w-full bg-white shadow-window-reverse-back z-10">
          {items.map((item) => (
            <li
              ...
              className="cursor-pointer hover:bg-window-header hover:text-white px-1 transition-none"
            >
              {item.text}
            </li>
          ))}
